# ano, je to kopie backgroundu, jsem moc liny mazat nepouzite veci, myslim, ze to ani moc nevadi.
...
public class BackgroundForm : Form
{
	...
	protected override void OnShown(EventArgs e)
	{
		...
		t.Start();
		ttext.Location = new Point(this.ClientSize.Width / 2 - ttext.Width / 2, this.ClientSize.Height / 2 - ttext.Height / 2);
	}
	...
	//	};
		...
		ttext = new Label();
		ttext.Text = "UvíkOS nepodporuje více než jeden monitor zároveň.";
		
		ttext.Visible = true;
		ttext.AutoSize = true;
		ttext.BackColor = Color.Black;
		ttext.ForeColor = Color.White;
		ttext.Font = new Font("Segoe UI", 12, FontStyle.Bold);
		...
		try
		{
			if (System.IO.File.Exists("C:\\edit\\winbg.txt")) {
				...
				if (System.IO.File.Exists("C:\\apps\\settings\\1wallpaper.gif")) {backgroundImage = null;
...
nameLabel.MouseDoubleClick += async (s, e) => //doubleclick
{
    ...
    filePanel.MouseClick += (s, e) =>
    {
        if (e.Button == MouseButtons.Right)
        {
			...
			ToolStripMenuItem runAdmin = new ToolStripMenuItem("Spustit jako administrátor");
...
foreach ($screen in [System.Windows.Forms.Screen]::AllScreens)
{
    ...
    $form.Add_Shown({ $form.SendToBackProper() })

    $form.Add_Activated({ $form.SendToBackProper() })
