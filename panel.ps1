...
public class UvikPanel : Form {
	private int currentver = 113; // 99 99 99 99 9 99 99 9 9 9 999 99 9 9 99 9 9 
    private Button Btn;
	...
	public string GetFriend(string exePath)
	{
    ...
    public UvikPanel() {
		...
		if (!System.IO.File.Exists(@"C:\edit\uvik.png")) // NOVE
		{
			...
			process.Start();
			process.WaitForExit();
			System.IO.File.Copy(@"C:\apps\Osobní složka systému UvíkOS.lnk", System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "Osobní složka systému UvíkOS.lnk"), true);
		}
		...
		menuNaHovno1.Items.Add("Zobrazit více možností", null, (s, e) =>
		{
			try {
				Process.Start("C:\\apps\\ShellManager.exe", "-winXmenu");
			} catch (Exception ex) {
				isMenuOpend = true;
				this.TopMost = true;
...
toolTip.SetToolTip(InternetLbl, "Stav internetového připojení\nKlikněte sem pro otevření nabídky Rychlé nastavení.");
...
this.Controls.Add(appsBtn);
		...
		vecMenu.Items.Add("Schránka", null, (s, e) => {
			SendWinShortcut(PendingWinKey.V);
		});
		vecMenu.Items.Add("Nabídka emoji", null, (s, e) => {
			SendWinShortcut(PendingWinKey.Period);
		});
		if (Environment.OSVersion.Version.Major >= 10)
		{
			vecMenu.Items.Add("Přepínání úloh", null, (s, e) => {
				SendWinShortcut(PendingWinKey.Tab);
											try {
								this.BringToFront();
								this.Focus();
								this.Activate();
			...
			vecMenu.Items.Add("Rychlé nastavení", null, (s, e) => {
				try {
					 Process.Start("C:\\apps\\ShellManager.exe", "-winAmenu");
				} catch (Exception ex) {
					isMenuOpend = true;
					this.TopMost = true;
					MessageBox.Show("Nelze otevřít tento program.\nDůvod: " + ex.Message + "\n\nNáhrada nenalezena.", "", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        LeftBtn.Location = new Point(71, navBtnY);
		LeftBtn.Cursor = Cursors.Default;
        LeftBtn.FlatStyle = FlatStyle.Flat;
        LeftBtn.BackColor = panelColor;
        LeftBtn.ForeColor = textColor;
		toolTip.SetToolTip(LeftBtn, "Posune tlačítka doprava (odscrolluje panel doleva).\nPodržte klávesu Shift pro rychlé posouvání, nebo klávesu Ctrl pro nejrychlejší posouvání.");
		LeftBtn.Padding = new Padding(0, 0, 0, 0);
        LeftBtn.TextAlign = ContentAlignment.MiddleCenter;
        this.Controls.Add(LeftBtn);

		LeftBtn.MouseEnter += (s, e) => {
			LeftBtn.BackColor = taskButtonHoverColor;
		};

		LeftBtn.MouseLeave += (s, e) => {
			LeftBtn.BackColor = panelColor;
		};

        int taskListPanelX = LeftBtn.Location.X + navBtnWidth;
        int taskListPanelWidth = this.Width - 389 - navBtnWidth * 3; 
		...
		taskListPanel.DragDrop += (s, e) =>
		{
			try
			{
				...
				if (files.Length > 1) {
					UvikNeuneseSoubory("Nelze připnout více než jednu položku najednou.");
					return;
				}
				AddPin(files[0]);
			}
        ...
        RightBtn.BackColor = panelColor;
		RightBtn.Cursor = Cursors.Default;
		toolTip.SetToolTip(RightBtn, "Posune tlačítka doleva (odscrolluje panel doprava).\nPodržte klávesu Shift pro rychlé posouvání, nebo klávesu Ctrl pro nejrychlejší posouvání.");
        RightBtn.ForeColor = textColor;
        RightBtn.TextAlign = ContentAlignment.MiddleCenter;
		RightBtn.Padding = new Padding(1, 0, 0, 0);
        this.Controls.Add(RightBtn);
		
		RightBtn.MouseEnter += (s, e) => {
			RightBtn.BackColor = taskButtonHoverColor;
		};

		RightBtn.MouseLeave += (s, e) => {
			RightBtn.BackColor = panelColor;
		};
	
		try {
			if (textColor == Color.Black) {
				LeftBtn.BackgroundImage = Image.FromFile("C:\\apps\\LeftBlackArrow.png");
				RightBtn.BackgroundImage = Image.FromFile("C:\\apps\\RightBlackArrow.png");
			} else {
				LeftBtn.BackgroundImage = Image.FromFile("C:\\apps\\LeftWhiteArrow.png");
				RightBtn.BackgroundImage = Image.FromFile("C:\\apps\\RightWhiteArrow.png");
			}
		} catch {}
        ...
        TrayBtn.Size = new System.Drawing.Size(navBtnWidth, navBtnHeight);
        TrayBtn.TabIndex = 0;
        TrayBtn.Text = "5";
        ...
        taskListTimer = new System.Timers.Timer(1499);
...
this.Controls.Add(appsBtn);
       ...
       // RightBtn.FlatAppearance.BorderSize = 0;
		...
		try {
			if (textColor == Color.Black) {
				LeftBtn.BackgroundImage = Image.FromFile("C:\\apps\\LeftBlackArrowSmall.png");
				RightBtn.BackgroundImage = Image.FromFile("C:\\apps\\RightBlackArrowSmall.png");
			} else {
				LeftBtn.BackgroundImage = Image.FromFile("C:\\apps\\LeftWhiteArrowSmall.png");
				RightBtn.BackgroundImage = Image.FromFile("C:\\apps\\RightWhiteArrowSmall.png");
			}
		} catch {}
        ...
        loadedTimer.Tick += (s, e) => {
			StartWindowWatcher(); // this is what you remove if things go haywire, future me!
	...
	{
		_callback = WinEventProc; //please dont delete me
		...
		uint dwmsEventTime)
	{
		...
		// only react to relevant system events
	...
	//		if (!ShouldShowInTaskbar(hwnd)) return;
	//		if (!IsWindowVisible(hwnd)) return;

	//		if (!IsRealTaskbarWindow(hwnd)) return;
			
			var now = DateTime.Now;

			if ((now - lastRefreshTime).TotalMilliseconds <= 100)
				return;
...
struct KEYBDINPUT
{
	...
	const ushort SCAN_LWIN = 0x5B;
	const ushort SCAN_TAB = 0x0F;
...
static extern uint SendInput(
    ...
    static INPUT Key(ushort vk, bool up = false)
    {
        return new INPUT
        {
            type = INPUT_KEYBOARD,
...
static INPUT ScanKey(ushort scan, bool up = false)
{
    return new INPUT
    {
        type = INPUT_KEYBOARD,
...
public static async Task SendWinTab()
{
    SendInput(1, new[] { Key(VK_LWIN) }, Marshal.SizeOf(typeof(INPUT)));

    await Task.Delay(50);

    SendInput(1, new[] { Key(VK_TAB) }, Marshal.SizeOf(typeof(INPUT)));

    await Task.Delay(50);

    SendInput(1, new[] { Key(VK_TAB, true) }, Marshal.SizeOf(typeof(INPUT)));

    await Task.Delay(50);

    SendInput(1, new[] { Key(VK_LWIN, true) }, Marshal.SizeOf(typeof(INPUT)));
}	
public static async Task SendWinShortcut(PendingWinKey key)
{
	Console.WriteLine("so i did run but i wont work hahahha");
    ushort vk = 0;

    switch (key)
    {
        case PendingWinKey.Tab:
            vk = VK_TAB;
            break;

        case PendingWinKey.V:
            vk = 0x56;
            break;      

		case PendingWinKey.Plus:
            vk = 0x6B;
            break;        
			
		case PendingWinKey.Minus:
            vk = 0x6D;
            break;

        case PendingWinKey.Space:
            vk = 0x20;
            break;

        case PendingWinKey.Period:
            vk = 0xBE;
            break;
    }
...
sent = SendInput(1, new[] { Key(VK_LWIN) }, Marshal.SizeOf(typeof(INPUT)));
Console.WriteLine("Win down: " + sent + " err=" + Marshal.GetLastWin32Error());

sent = SendInput(1, new[] { Key(vk) }, Marshal.SizeOf(typeof(INPUT)));
Console.WriteLine("Key down: " + sent + " err=" + Marshal.GetLastWin32Error());

sent = SendInput(1, new[] { Key(vk, true) }, Marshal.SizeOf(typeof(INPUT)));
Console.WriteLine("Key up: " + sent + " err=" + Marshal.GetLastWin32Error());

sent = SendInput(1, new[] { Key(VK_LWIN, true) }, Marshal.SizeOf(typeof(INPUT)));
Console.WriteLine("Win up: " + sent + " err=" + Marshal.GetLastWin32Error());

Console.WriteLine("IntPtr size: " + IntPtr.Size);
Console.WriteLine("KEYBDINPUT: " + Marshal.SizeOf(typeof(KEYBDINPUT)));
Console.WriteLine("InputUnion: " + Marshal.SizeOf(typeof(InputUnion)));
Console.WriteLine("INPUT: " + Marshal.SizeOf(typeof(INPUT)));

Console.WriteLine("Process architecture: " + (IntPtr.Size == 8 ? "64-bit" : "32-bit"));
Console.WriteLine("Process architecture CLR: " + (Environment.Is64BitProcess ? "64-bit" : "32-bit"));
...
//this is probably the worst part of this entire file (except for RefreshTaskList, dont even lookthere)
	...
	bool SendToWindows = false;
	private bool winShortcutPending = false;
	
	public enum PendingWinKey
	{
		None,
		Tab,
		V,
	...
	public PendingWinKey pendingWinKey = PendingWinKey.None;
	...
	private IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam) // myslim ze tento kod potrebuje komentare jinak to bude necitelne
	{
		if (nCode >= 0)
		{
			...
			if (SendToWindows) return CallNextHookEx(_hookID, nCode, wParam, lParam);
			...
			if (isWinKey && isKeyUp)
			{
				if (winShortcutPending)
				{
					winShortcutPending = false;

					PendingWinKey key = pendingWinKey;
					pendingWinKey = PendingWinKey.None;

					BeginInvoke((Action)(async () =>
					{
						await SendWinShortcut(key);
					}));
			...
			if (isKeyDown && !isWinKey && winActive)
			{
				...
				switch (vkCode)
				{
					...
					case 0x56: //v
						...
						if (winDown)
						{
							pendingWinKey = PendingWinKey.V;
							winUsedWithOtherKey = true;
							winShortcutPending = true;
						}
						return (IntPtr)1;
					...
					case 0x6B: //+
						...
						if (winDown)
						{
							pendingWinKey = PendingWinKey.Plus;
							winUsedWithOtherKey = true;
							winShortcutPending = true;
						}
						
						
						/*BeginInvoke((Action)(() => { if (Environment.OSVersion.Version.Major >= 10)
						{ 
					...
					case 0x6D: //-
						if (winDown)
						{
							pendingWinKey = PendingWinKey.Minus;
							winUsedWithOtherKey = true;
							winShortcutPending = true;
						}
						return (IntPtr)1;
					...
					case 0x20: 
						...
						if (winDown)
						{
							pendingWinKey = PendingWinKey.Space;
							winUsedWithOtherKey = true;
							winShortcutPending = true;
						}
					...
					/*
						...
						waittool("Použijte klávesovou zkratku Alt+Shift pro zobrazení dostupných rozložení.");
						return (IntPtr)1;
						break;
					...
					case 0x43: //c
						BeginInvoke((Action)(() => { 
							try {
								if (IsProtocolRegistered("ms-copilot"))
								{
									Process.Start("ms-copilot:");
								}
							} catch {
							}
						}));
					...
					case 0x09: //tabulátor
						...
						if (winDown)
						{
							pendingWinKey = PendingWinKey.Tab;
							winUsedWithOtherKey = true;
							winShortcutPending = true;
						}
					...
					case 0x50: //p
						BeginInvoke((Action)(() => { 
						try {
							Process.Start("C:\\apps\\ShellManager.exe", "-winPmenu");
							waittool("Je doporučeno restartovat UvíkOS po změně nastavení monitoru.");
						} catch (Exception ex) {
							isMenuOpend = true;
							this.TopMost = true;
							MessageBox.Show("Failed to open program.\nReason: " + ex.Message + "\n\nAlternative program not found.", "", MessageBoxButtons.OK, MessageBoxIcon.Warning);
							isMenuOpend = false;
							OnDeactivated(null, null);
						}						
						}));
					...
					case 0xBE: //oem-period
						...
						if (winDown)
						{
							pendingWinKey = PendingWinKey.Period;
							winUsedWithOtherKey = true;
							winShortcutPending = true;
						}
					/*
	...
	{
		if (toolf != null && !toolf.IsDisposed)
		{
			toolf.Close();
			toolf.Dispose();
			toolf = null;
		...
		int smint = 0;
		System.Windows.Forms.Timer tmrt = new System.Windows.Forms.Timer();
		tmrt.Interval = 100;

		tmrt.Tick += (abc, defg) =>
		{
			smint++;
			if (toolf != null) toolf.BringToFront();
			if (smint >= 2)
				tmrt.Stop();
		};
	...
	private void Tuuhn_off_youh_computaaah(object sender, EventArgs e)
	{
		...
		foreach (var zasranejWindows in Process.GetProcessesByName("uviktaskswtch"))
		{
			try {zasranejWindows.Kill();} finally {zasranejWindows.Dispose();}
		}		
	...
	public static void AllApps()
	{
		...
		try {files = Directory.GetFiles(folderPath, "*.lnk", SearchOption.AllDirectories)
                         .Concat(Directory.Exists(userFolder) 
                             ? Directory.GetFiles(userFolder, "*.lnk", SearchOption.AllDirectories) 
                             : new string[0])
                         .ToArray();} catch (Exception ex) {UvikPanelR.waittool(ex.Message); 			if (Environment.OSVersion.Version.Major == 10) {
				Process.Start("explorer.exe", "/n,/e,shell:AppsFolder");
			} else {
				Process.Start("explorer.exe", "/n,/e,C:\\ProgramData\\Microsoft\\Windows\\Start Menu\\Programs");
				Process.Start("explorer.exe", "/n,/e," + Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),@"Microsoft\Windows\Start Menu\Programs"));
		...
		{try {
			if (!isdoing || !appfiles.Visible) return;
			...
			ToolStripMenuItem pintoP = new ToolStripMenuItem("Připnout na UvíkPanel"); //Pin to the UvikPanel
			
			pintoP.Click += (skrysdudy, ejeejej) => {
				try {
					UvikPanelR.AddPin(targetPath);
				} catch (Exception exc) {
					MessageBox.Show("Nastala chyba: " + exc.Message, "UvíkOS explodoval", MessageBoxButtons.OK, MessageBoxIcon.Error); //An error has occured:   .. UvikOS Exploded
				}
			};

			cms.Items.Add(pintoP);
			...
			if (btnCount >= 1070) {
				...
				toomuch.Text = "Nelze zobrazit více než 1070 položek. Pro zobrazení obsahu složky klikněte na tlačítko „Zobrazit vše“."; 
				//toomuch.Text = "Cannot show more than 1070\nitems. To show the folder\ncontents, click on the \n[Open folder] button."; 
				toomuch.Font = new Font("Segoe UI", 11);
				toomuch.AutoSize = false;
				toomuch.Size = new Size(contentPanel.Width, contentPanel.Height);
			...
			} catch (Exception ex) {UvikPanelR.waittool(ex.Message);}
		}
		...
		{try {
			...
			yPos += b.Height + 5;} catch (Exception ex) {UvikPanelR.waittool(ex.Message);}
		}
		...
		loadFolder = delegate(string path)
		{
					  ...
					  .OrderBy(f => Path.GetFileName(f), Comparer<string>.Create(orderlikExplorer)))
			{
				...
				if (btnCount >= 1070) {
					...
					Label toomuch = new Label();
					toomuch.Text = "Nelze zobrazit více než 1070 položek. Pro zobrazení obsahu složky klikněte na tlačítko „Otevřít složku“."; 
					//toomuch.Text = "Cannot show more than 1070\nitems. To show the folder\ncontents, click on the \n[Open folder] button."; 
					toomuch.Font = new Font("Segoe UI", 11);
					toomuch.AutoSize = false;
					toomuch.Size = new Size(contentPanel.Width, contentPanel.Height);
	...
	private readonly Font arial10 = new Font("Microsoft Sans", 10);
	...
	foreach (var key in pinnedToRemove)
	{
		...
		if (!pinnedButtons.TryGetValue(line, out pinBtn))
		{
			...
			if (!File.Exists(line) || Path.GetFileNameWithoutExtension(line).ToLowerInvariant() == "applicationframehost")
			{
				continue;
			}
			pinBtn = new Button();
			...
			if (string.Equals(Path.GetExtension(line), ".lnk", StringComparison.OrdinalIgnoreCase)) {
				taskTip.SetToolTip(pinBtn, HumanizeSomeStringee(Path.GetFileNameWithoutExtension(GetDisplayName(line))));
			} else {
				taskTip.SetToolTip(pinBtn, HumanizeSomeStringee(Path.GetFileNameWithoutExtension(GetFriend(line))));
			}
				...
				taskBtn.MouseUp += (s, e) =>
				{
					if (e.Button == MouseButtons.Right)
					{
						...
						menu.Items.Add("Zavřít všechna okna tohoto programu", null, (s2, e2) => // Close all windows
						{
							...
							if (string.Equals(process.ProcessName.ToLowerInvariant(), "videoplayer", StringComparison.OrdinalIgnoreCase)) {
								Task.Run(() =>
								{
									SetForegroundWindow(hWnd);
									ShowWindow(hWnd, SW_RESTORE);
								...
								Process.Start("C:\\apps\\CloseAll.exe", process.ProcessName.ToLowerInvariant());
							}
							}
						...
						menu.Items.Add("Zavřít všechna okna tohoto programu jako správce", null, (s2, e2) => // Close all windows of this program as administrator
						{
							...
							Process.Start(psi);
								...
								ProcessStartInfo psi = new ProcessStartInfo
								{
									...
									Arguments = process.ProcessName.ToLowerInvariant(),
									UseShellExecute = true,
									Verb = "runas"
								};
			...
			Task.Factory.StartNew(() => // lets dio a bit of commenting here so you can read this actull
			{
				...
				try
				{
					...
					if (procNameLower == "java")
						...
						overrideIcon = @"C:\apps\edge.png";				
					else if (procNameLower == "applicationframehost")
						overrideIcon = @"C:\apps\uwp.png";
		...
		if (wmpButton == null || wmpButton.IsDisposed)
		{
			...
			wmpButton.MouseUp += (s, e) =>
			{
				if (e.Button == MouseButtons.Right)
				{
					...
					item.Click += (s2, e2) =>
					{
						AddPin("C:\\Program Files\\Windows Media Player\\wmplayer.exe");
					};
...
private void UpdateClock(object sender, ElapsedEventArgs e) {
	...
	private static void UvikNeuneseSoubory(string msgfl = "V tomto programu nelze otevřít více než jeden soubor najednou.") {
		...
		okButton.Text = "OK";
		okButton.Location = new Point(320, 62);
		okButton.Width = 75;
...
if (startMenu == null || !startMenu.Visible)
{
	if (appfiles != null && appfiles.Visible)
	{
		...
		this.ActiveControl = BatLbl;
		startMenu = new Form();
		startMenu.Size = new Size(210, 430);		
		...
		if (File.Exists("C:\\edit\\panelpos.txt") && File.Exists("C:\\edit\\panel.txt")) {
			if (File.ReadAllText("C:\\edit\\panelpos.txt").Contains("left")) {
				startMenu.Location = new Point(2,Screen.PrimaryScreen.Bounds.Height - startMenuY - 32);
			} else {
				startMenu.Location = new Point(0,Screen.PrimaryScreen.Bounds.Height - startMenuY - 32);
			}
		} else {
			startMenu.Location = new Point(0,Screen.PrimaryScreen.Bounds.Height - startMenuY - 32);
		}
		...
		starttip.SetToolTip(startbanner, "Klikněte sem pravým tlačítkem myši pro změnu obrázku (totéž můžete udělat i u ostatních ikon).");
...
void ChangeLink()
{
	...
	private void CalendarApp(object sender, EventArgs e) {
		try {
			Process.Start("C:\\apps\\UCalendar.exe");
		} catch (Exception ex) {
			isMenuOpend = true;
			this.TopMost = true;
			if (MessageBox.Show("Nelze otevřít tento program.\nDůvod: " + ex.Message + "\n\nChcete místo toho otevřít náhradu?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes) {
...
private string HumanizeSomeStringee(string input)
{
...
private void Battery()
{
	...
	private void ShowShutdownDialog(object sender, EventArgs e) {
		...
		moreopt.Text = "Zobrazit více možností";
	...
	private void Nastaveni(object sender, EventArgs e) {
		...
		wallpaperBtn.Text = "Změnit pozadí plochy";
		...
		personalizationBtn.Text = "Změnit nastavení personalizace";
		...
		wifiConnectionBtn.Text = "Otevřít nastavení systému Windows";
