...
function openApp(appName) {
    ...
    createWindow(appName, '<iframe src="' + urls[appName] + '" width="100%" height="200%"></iframe>');
}
...
function createWindow(title, content) {
    ...
    windowDiv.innerHTML = 
        '<div class="window-titlebar">' +
            '<marquee><span>' + title + '</span></marquee>' +
            '<marquee><button onclick="closeWindow(this)">X</button></marquee>' +
        '</div>' +
        ...
        if (file) {
            ...
            reader.onload = function(e) {
                ...
                if (fileType.indexOf('image') !== -1) {
                    createWindow(file.name, '<marquee><img src="' + contentUrl + '" alt="' + file.name + '" style="width:100%;height:100%;"></marquee>');
                } else if (fileType === 'text/plain') {
                    createWindow(file.name, '<marquee><pre style="white-space: pre-wrap;">' + e.target.result + '</pre></marquee>');
                } else if (fileType === 'text/html') {
                    createWindow(file.name, '<marquee><iframe src="' + contentUrl + '" width="100%" height="100%"></iframe></marquee>');
                } else {
